...
CLASS lcl_incassi_ampre_leaseco IMPLEMENTATION.
  ...
  METHOD data_checks.
    ...
    LOOP AT mt_incasso ASSIGNING FIELD-SYMBOL(<ls_incasso>).
      ...
      IF lv_prev IS INITIAL OR lv_prev <> <ls_incasso>-altkn.
        lv_prev = <ls_incasso>-altkn.
        DATA(lv_buzei) = 1.
      ENDIF.
  ...
  METHOD excel_to_itab.
    ...
    DATA(lv_string) = p_file.  " Example string
    SHIFT lv_string LEFT BY ( strlen( lv_string ) - 4 ) PLACES IN CHARACTER MODE.
  ...
  METHOD xlsx_to_itab.
    ...
    IF lt_worksheet IS INITIAL.
      MESSAGE 'No worksheet was found. Please, check your Excel file!'(014)
         TYPE 'S' DISPLAY LIKE 'E'.
      LEAVE LIST-PROCESSING.
    ENDIF.
